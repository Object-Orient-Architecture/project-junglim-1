...
# 과제1
bbox = space.GetBoundingBox(geo.Plane.WorldXY)
...
for i in range(x_count):
    for j in range(y_count):
        ...
        if is_pt_inside_curve(point,space):
            points.append(point)
...
height = 5
rec_extrusion = [geo.Extrusion.Create(rectangles[i], height, True) for i in range(len(rectangles))]